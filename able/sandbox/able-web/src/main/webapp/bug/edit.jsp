...
<stripes:form action="/Bug.action">
  ...
  <table class="display">
    ...
    <tr>
      <th>Opened On:</th>
      <td><fmt:formatDate value="${actionBean.entity.openDate}" dateStyle="medium" /></td>
    </tr>
    <tr>
      <th><stripes:label for="entity.priority">Priority</stripes:label>:</th>
      <td><bug:fieldEditPriority /></td>
    </tr>
    <tr>
      <th><stripes:label for="entity.component.id">Component</stripes:label>:</th>
      <td><stripes:select name="entity.component.id">
        <stripes:options-collection collection="${actionBean.allValues.component}" label="name" value="id" />
      </stripes:select></td>
    </tr>
    <tr>
      <th><stripes:label for="entity.owner.id">Owner</stripes:label>:</th>
      <td><stripes:select name="entity.owner.id">
        <stripes:options-collection collection="${actionBean.allValues.owner}" label="username" value="id" />
      </stripes:select></td>
    </tr>
    <tr>
      <th><stripes:label for="entity.status">Status</stripes:label>:</th>
      <td><c:forEach var="status" items="${actionBean.allValues.status}">
        <stripes:radio id="entity.status.${stripes:enumName(status)}" name="entity.status" value="${stripes:enumName(status)}"
          checked="New" />
