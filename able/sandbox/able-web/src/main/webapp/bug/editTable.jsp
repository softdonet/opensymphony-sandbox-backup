...
<stripes:form action="/Bug.action">
  ...
  <table class="display">
    ...
    <c:forEach items="${actionBean.bulkEditEntities}" var="row" varStatus="rowstat">
      <tr>
        <td>${row.id}</td>
        <td><stripes:select name="bulkEditEntities[${rowstat.index}].priority" value="Medium">
          <stripes:options-collection collection="${actionBean.allValues.priority}" />
        </stripes:select></td>
        <td><stripes:select name="bulkEditEntities[${rowstat.index}].component.id">
          <stripes:options-collection collection="${actionBean.allValues.component}" label="name" value="id" />
        </stripes:select></td>
        <td><stripes:select name="bulkEditEntities[${rowstat.index}].owner.id">
